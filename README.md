...
| 路径 | 内容 | 读写 |
| --- | --- | --- |
| `~/.claude/` | Claude Code 自己的数据：会话记录、配置、插件 | ccockpit **只读**，仅「设置」页显式保存时写 `settings.json` |
| `~/.claude.json` | Claude Code 主配置 | **永不写入**，只读取展示 |
| `~/.ccockpit/index.db` | ccockpit 的索引（SQLite） | 可随时删除，重启后会重建 |
| `~/.ccockpit/config.json` | ccockpit 自身设置（端口、扫描目录等） | 由「系统」页写入 |
| `~/.ccockpit/auth.json` | 访问令牌的哈希 | 由「系统」页写入 |

索引是纯派生数据。任何时候觉得统计不对，删掉 `index.db` 重启即可重建，不会碰你的原始记录。

## 远程访问

默认只监听 `127.0.0.1`，别的机器连不上。要从别的机器访问（比如 ccockpit 跑在服务器上），
...
忘记令牌时，在服务器上删掉 `~/.ccockpit/auth.json` 即可恢复无认证状态。
...
`~/.ccockpit/config.json` 里改 `port`，或者删掉这个文件恢复默认 7433。
