...
export function System() {
  ...
  async function rescan(full: boolean) {
    if (full && !window.confirm("完全重建将清空索引后重扫全部数据,继续?")) return;
    setBusy(true);
    try {
  ...
  return (
    <div className="max-w-3xl">
      ...
      <section className="mt-8 rounded-2xl border border-line bg-panel p-6 shadow-[0_1px_2px_rgb(0_0_0/0.03)]">
        ...
        {!status ? (
          <p className="mt-5 text-sm text-muted">连接服务中…</p>
        ) : (
          <div className="mt-5 space-y-5">
            ...
            <dl className="grid grid-cols-2 gap-x-8 gap-y-4 sm:grid-cols-4">
              <Stat label="文件" value={`${status.filesDone} / ${status.filesTotal}`} />
              <Stat label="字节" value={`${fmtBytes(status.bytesDone)} / ${fmtBytes(status.bytesTotal)}`} />
              <Stat label="解析错误" value={String(status.errors)} />
              <Stat
                label="上次完成"
